...
Wird für die Testfunktion $\delta \bm{u}_h$ und die Verschiebung $\bm{u}_h$ sowie die diskrete Darstellung der Geometrie $\bm{x}_h$ der gleiche Ansatz verwendet, so spricht man von einem isoparametrischen Element:
